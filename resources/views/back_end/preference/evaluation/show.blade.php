        ...
        <div class="card">
            ...
            <div class="card-header">
                <div class="align-items-center">
                    ...
                    <div>
                    <a href="{{ route('evaluation.create_with_param', ['defense_enrollment_id' => $defenseEnrollment->id, 'jury_id' => $juryMember->id]) }}" class="btn btn-sm btn-primary btn-round ml-auto">
                        <i class="fa fa-plus"></i> Add New Evaluation
                    </a>
                    </div>

                    <div>
                    <form action="{{ route('evaluation.delete_all', ['defense_enrollment_id' => $defenseEnrollment->id, 'jury_id' => $juryMember->id]) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')

                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete all evaluations for this jury member? This action cannot be undone.');">
                                ...
                                @foreach ($defenseEnrollment->evaluations as $evaluation)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <!-- EDIT BUTTON -->
                                            <a href="{{ route('evaluation.edit', $evaluation->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            ...
                                            <form action="{{ route('evaluation.destroy', $evaluation->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this evaluation?');">
                                                    Delete
                                                </button>
                                            </form>
                                        </td>
            ...
            <!-- Card Footer -->
            <div class="">
                <a href="{{ route('evaluation.index') }}" class="btn btn-primary">Back to Evaluations</a>
            </div>
